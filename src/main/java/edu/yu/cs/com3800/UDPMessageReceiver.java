...
public class UDPMessageReceiver extends Thread implements LoggingServer {
    ...
    public UDPMessageReceiver(LinkedBlockingQueue<Message> incomingMessages,
                              ...
                              int myPort,
                              PeerServer peerServer) throws IOException {
        ...
        this.logger = Util.initializeLogging("UDPMessageReceiver", peerServer.getServerId(), myPort, "BASIC");
    }
    ...
    @Override
    public void run() {
        logger.info("Starting UDPMessageReceiver on port " + this.myPort);
        //create the socket
        DatagramSocket socket = null;
        try {
            logger.info("Creating receiving socket on port " + this.myAddress.getPort());
            socket = new DatagramSocket(this.myAddress);
            socket.setSoTimeout(3000);
        }
        catch (Exception e) {
            logger.log(Level.SEVERE, "Failed to create receiving socket on port " + this.myAddress.getPort(), e);
            return;
        }
        //loop
        while (!this.isInterrupted()) {
            try {
                ...
                if (!this.isInterrupted()) {
                    logger.log(Level.WARNING, "Exception caught while trying to receive UDP packet", e);
                }
            }
        }
        //cleanup
        ...
        logger.info("Exiting UDPMessageReceiver");
    }
    ...
    private void sendElectionReply(byte[] msgContent, InetSocketAddress target) {
        ...
        try (DatagramSocket socket = new DatagramSocket()){
            ...
            logger.warning("Failed to send election reply:\n" + msg.toString());
        }
    }
